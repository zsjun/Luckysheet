...
const luckysheetformula = {
  ...
  errorParamCheck: function(thisp, data, i) {
    ...
    if (type.indexOf("all") > -1) {
      ...
      if (
        ...
        (getObjType(data) == "object" || getObjType(data) == "array")
      ) {
        return [true, locale_formulaMore.tipSuccessText];
      }
      ...
      if (
        ...
        (isRealNum(data) || getObjType(data) == "boolean")
      ) {
        return [true, locale_formulaMore.tipSuccessText];
      }
  ...
  getValueByFuncData: function(value, arg) {
    ...
    if (getObjType(value) == "array") {
      if (arg == "avg") {
        return luckysheet_function.AVERAGE.f.apply(
          luckysheet_function.AVERAGE,
          value
        );
      } else if (arg == "sum") {
  ...
  createRangeHightlight: function() {
    ...
    let $span = $("#luckysheet-rich-text-editor").find(
  ...
  searchHTML:
    '<div id="luckysheet-formula-search-c" class="luckysheet-formula-search-c"></div>',
  helpHTML:
  ...
  getrangeseleciton: function() {
    ...
    if (anchor.parent().is("span") && anchorOffset != 0) {
      ...
      anchor.is("#luckysheet-functionbox-cell")
    ) {
      let txt = $.trim(
        anchor
          .find("span")
  ...
  searchFunctionEnter: function($obj) {
    ...
    _this.searchFunctionCell
      .text(functxt)
      .after('<span dir="auto" class="luckysheet-formula-text-color">(</span>');
    _this.setCaretPosition(_this.searchFunctionCell.next().get(0), 0, 1);
    $("#luckysheet-formula-search-c").hide();
    _this.helpFunctionExe(
      _this.searchFunctionCell.closest("div"),
      _this.searchFunctionCell.next()
    );
  },
  ...
  helpFunction: function($editer, funcname, paramIndex) {
    ...
    let $func =
      functionlist[_this.functionlistPosition[$.trim(funcname).toUpperCase()]];
    if ($func == null) {
      return;
    }
    ...
    $(
      "#luckysheet-formula-help-c .luckysheet-arguments-help-function-name"
    ).html($func.n);
    $(
      "#luckysheet-formula-help-c .luckysheet-arguments-help-parameter-content"
    ).html($func.d);
    ...
    let helpformulaItem =
      '<span class="luckysheet-arguments-help-parameter" dir="auto">${param}</span>';
    let helpformulaArg =
    ...
    for (let i = 0; i < $func.p.length; i++) {
      ...
      fht +=
        '<span class="luckysheet-arguments-help-parameter" dir="auto">' +
        name +
        "</span>, ";
      ahf +=
        '<span class="luckysheet-arguments-help-parameter" dir="auto">' +
    ...
    $(
      "#luckysheet-formula-help-c .luckysheet-formula-help-title .luckysheet-arguments-parameter-holder"
    ).html(fht); //介绍
    $(
      "#luckysheet-formula-help-c .luckysheet-arguments-help-formula .luckysheet-arguments-parameter-holder"
    ).html(ahf); //示例
    $("#luckysheet-formula-help-c .luckysheet-formula-help-content-param").html(
      fhcp
    ); //参数
  ...
  rangeHightlightselected: function($editer, kcode) {
    ...
    if (
      $(currSelection).closest(".luckysheet-formula-functionrange-cell")
        .length == 0
    ) {
    ...
    let $anchorOffset = $(currSelection).closest(
  ...
  iscelldata: function(txt) {
    ...
    if (rangetxt.indexOf(":") == -1) {
      ...
      if (
        rangetxt[0].toString().match(reg_cellRange) &&
  ...
  setfreezonFuc: function(event) {
    ...
    if (rangetxtIndex == -1) {
      ...
      if (pos > rangetxtIndex) {
        let ret =
          prefix + rangetxt[0] + ":" + _this.setfreezonFuceExe(rangetxt[1]);
        newtxt = ret;
        newpos = ret.length;
      } else {
  ...
  updateparam: function(orient, txt, step) {
    ...
    if (rangetxt.indexOf(":") == -1) {
      ...
      if (isNaN(col[0]) && isNaN(col[1])) {
        ...
        return (
          prefix + $col0 + chatatABC(col[0]) + ":" + $col1 + chatatABC(col[1])
        );
      } else {
        return (
          prefix +
  ...
  functionStrChange: function(txt, type, rc, orient, stindex, step) {
    ...
    while (i < funcstack.length) {
      ...
      if (s == "(" && matchConfig.dquote == 0) {
        ...
        matchConfig.bracket -= 1;
        function_str +=
          _this.functionStrChange(str, type, rc, orient, stindex, step) + ")";
        str = "";
      } else if (s == '"' && matchConfig.squote == 0) {
        if (matchConfig.dquote > 0) {
          function_str += str + '"';
        ...
        if (s + s_next in _this.operatorjson) {
          ...
          if (str.length > 0) {
            function_str +=
              _this.functionStrChange(str, type, rc, orient, stindex, step) + s;
            str = "";
          } else {
            function_str += s;
          }
      ...
      if (i == funcstack.length - 1) {
        if (_this.iscelldata($.trim(str))) {
          function_str += _this.functionStrChange_range(
            $.trim(str),
            type,
            rc,
            orient,
  ...
  rangeSetValue: function(selected, obj) {
    ...
    if (
      Store.config["merge"] != null &&
      rf + "_" + cf in Store.config["merge"]
    ) {
      range = getRangetxt(
    ...
    if (
      _this.rangestart ||
      _this.rangedrag_column_start ||
      _this.rangedrag_row_start
    ) {
      if (
        ...
        if (isVal) {
          //公式计算
          let fp = $.trim(
            _this.functionParserExe($("#luckysheet-rich-text-editor").text())
          );
        ...
        let $span = $editor
          .find("span[rangeindex='" + _this.rangechangeindex + "']")
          .html(range);
  ...
  rangedrag: function(event) {
    ...
    if (_this.func_selectedrange.top > row_pre) {
      top = row_pre;
      height =
        _this.func_selectedrange.top +
        _this.func_selectedrange.height -
        row_pre;
    ...
    if (_this.func_selectedrange.left > col_pre) {
      left = col_pre;
      width =
        _this.func_selectedrange.left +
        _this.func_selectedrange.width -
        col_pre;
  ...
  rangeResizeDraging: function(
    ...
    if (
      luckysheetCurrentChartResize == "lt" ||
      luckysheetCurrentChartResize == "lb"
    ) {
      if (
        luckysheetCurrentChartResizeXy[0] + luckysheetCurrentChartResizeXy[2] <
        col_pre
      ) {
        return;
      }
      ...
      if (
        ...
        left = 0;
        width =
          luckysheetCurrentChartResizeXy[2] + luckysheetCurrentChartResizeXy[0];
      }
    }

    if (
      luckysheetCurrentChartResize == "rt" ||
      luckysheetCurrentChartResize == "rb"
    ) {
      if (
        luckysheetCurrentChartResizeXy[6] - luckysheetCurrentChartResizeXy[2] >
        col
      ) {
        return;
      }
      ...
      width =
        luckysheetCurrentChartResizeXy[2] +
        col -
        luckysheetCurrentChartResizeXy[6];
    ...
    if (
      ...
      if (
        luckysheetCurrentChartResizeXy[1] + luckysheetCurrentChartResizeXy[3] <
        row_pre
      ) {
        return;
      }
      ...
      if (
        ...
        top = 0;
        height =
          luckysheetCurrentChartResizeXy[3] + luckysheetCurrentChartResizeXy[1];
      }
    }
    ...
    if (
      ...
      if (
        luckysheetCurrentChartResizeXy[7] - luckysheetCurrentChartResizeXy[3] >
        row
      ) {
        return;
      }
      ...
      height =
        luckysheetCurrentChartResizeXy[3] +
        row -
        luckysheetCurrentChartResizeXy[7];
    ...
    let $span = _this.rangeResizeTo
      .find("span[rangeindex='" + rangeindex + "']")
      .html(rangetxt);
  ...
  rangeMoveDraging: function(
    ...
    let row_s =
        luckysheet_select_save["row"][0] -
        row_index_original +
        rowLocation(y)[2],
      row_e =
        luckysheet_select_save["row"][1] -
        row_index_original +
        rowLocation(y)[2];
    let col_s =
    ...
    if (row_s < 0 || y < 0) {
      row_s = 0;
      row_e =
        luckysheet_select_save["row"][1] - luckysheet_select_save["row"][0];
    }
    if (col_s < 0 || x < 0) {
      col_s = 0;
      col_e =
  ...
  // $to 目的dom，$input 输入的dom，kcode 输入的字符
        ...
        if (s + s_next in _this.operatorjson) {
          if (bracket.length == 0) {
            if ($.trim(str).length > 0) {
              cal2.unshift(
                _this.functionParser($.trim(str), cellRangeFunction)
              );
      ...
      if (i == funcstack.length - 1) {
        ...
        if (_this.iscelldata(str_nb) && str_nb.substr(0, 1) != ":") {
          ...
          if (_this.iscelldata(str_nb)) {
            endstr =
              "luckysheet_getSpecialReference(false," +
              function_str +
              ",'" +
              str_nb +
              "')";
  ...
  isFunctionRangeSimple1: function(txt, r, c, index, dynamicArray_compute) {
    ...
    while (i < funcstack.length) {
      ...
      if (s == "(" && matchConfig.dquote == 0) {
        ...
        matchConfig.bracket -= 1;
        function_str +=
          _this.isFunctionRangeSimple(str, r, c, index, dynamicArray_compute) +
          ")";
        str = "";
      } else if (s == "," && matchConfig.dquote == 0) {
        //matchConfig.comma += 1;
      ...
      if (i == funcstack.length - 1) {
        if (_this.iscelldata($.trim(str))) {
          _this.isFunctionRangeSaveChange(
            str,
            r,
            c,
            index,
      ...
      if (isOffsetFunc) {
        ...
        if (isoff == true) {
          for (let cellStr in cellList) {
            ...
            this.isFunctionRangeSaveChange(
              cellStr,
              r,
              c,
              index,
          ...
          this.isFunctionRange(txt, r, c, index, dynamicArray_compute, function(
            str
          ) {
  ...
  isFunctionRange: function(
    ...
    while (i < funcstack.length) {
      ...
      if (
        ...
        if (bracket.length == 0) {
          ...
          let functionS = _this.isFunctionRange(
            str,
            r,
            c,
            index,
    ...
    // console.log(function_str);
    _this.checkSpecialFunctionRange(
      function_str,
      r,
      c,
      index,
  ...
  checkSpecialFunctionRange: function(
    function_str,
    r,
    c,
    index,
  ...
  execFunctionGroup: function(
    ...
    if (value != null) {
      ...
      _this.execFunctionGlobalData[origin_r + "_" + origin_c + "_" + index] =
        cellCache[0][0];
    }
    ...
    for (let i = 0; i < calcChains.length; i++) {
      ...
      let key =
        "r" + formulaCell.r + "c" + formulaCell.c + "i" + formulaCell.index;
      let calc_funcStr = getcellFormula(
        formulaCell.r,
        formulaCell.c,
        formulaCell.index
      );
  ...
  execFunctionGroup1: function(
    ...
    let dynamicArray_compute =
      luckysheetfile[getSheetIndex(Store.currentSheetIndex)][
        "dynamicArray_compute"
      ] == null
        ? {}
        : luckysheetfile[getSheetIndex(Store.currentSheetIndex)][
      ...
      for (let i = 0; i < group.length; i++) {
        ...
        if (cell != null && cell.f != null && cell.f == calc_funcStr) {
          ...
          if (_this.isFunctionRangeSave) {
            stack.push(item);
            _this.execvertex[
              "r" + item.r + "c" + item.c + "i" + item.index
            ] = item;
            count++;
          }
        }
    ...
    if (dynamicArray != null) {
      for (let i = 0; i < dynamicArray.length; i++) {
        ...
        if (
          calc.r == r &&
          calc.c == c &&
          (calc.index == null || calc.index == index)
        ) {
  ...
  execfunction: function(txt, r, c, index, isrefresh, notInsertFunc) {
    ...
    //console.log(fp)
    if (
      fp.substr(0, 20) == "luckysheet_function." ||
